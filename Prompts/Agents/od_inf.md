...
The sandbox at `{{paths.sandbox}}` is the message bus between agents. All inter-agent data exchange happens via file paths, never raw content passed through the orchestrator layer.

- Producing agent writes output to `{{paths.sandbox}}` under an appropriate filename and returns only the file path
