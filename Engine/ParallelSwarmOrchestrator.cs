...
public static class ParallelSwarmOrchestrator
{
    ...
    private static string InjectRetryHint(string task, int attemptNumber, string? lastFailureReason)
    {
        ...
        {
            2 => SkillUsageGuidance.RetryHint,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var nt in NativeToolRegistry.BuildPool(App.Config)) mcpTools.Add(nt);
        ...
        await MuxConsole.WithSpinnerAsync("Initializing specialist agents", async () =>
        {
            foreach (var def in agentDefs)
            {
                ...
                var readSkillTool = AIFunctionFactory.Create(
                    method: (
                        [Description(SkillUsageGuidance.SkillNameDescription)] string skillName
                    ) =>
                    {
        ...
        if (cancellationToken.IsCancellationRequested)
            OtelMetrics.SessionsFailed.Add(1, new KeyValuePair<string, object?>("mode", "pswarm"));
        else
            OtelMetrics.SessionsCompleted.Add(1, new KeyValuePair<string, object?>("mode", "pswarm"));
    }
        ...
        bool prodMode = false)
    {
        ...
        using var _replScope = ReplShellTools.BeginScope(
