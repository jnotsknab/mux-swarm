...
public struct Help
{
    public static readonly string HelpText = string.Join("\r\n",
        ...
        "",
        "Slash Commands",
        "  /agent          Open the main agentic interface with the selected lead agent (start here)",
        "                  Work directly or delegate; ultra with delegation, giga, and team leads can match or exceed swarm modes.",
        "  /swarm          Specialty: serial coordinator dispatch, one specialist at a time",
        "  /pswarm         Specialty: coordinator-led concurrent batches of independent specialists",
        ...
        "  /createhook      Guided wizard to scaffold a hook (script/command, event, agent/tool filter, mode) into swarm.json hooks[]",
        ...
        "  /resume         Resume a previous lead-agent session; bare /resume opens the fuzzy picker (Enter resume, v preview)",
        "  /history        Browse past sessions in a full-screen viewer: fuzzy find, Enter opens, j/k scroll, / searches",
        "  /telemetry      Start the telemetry dashboard (tokens/cost/tools, all-time, Grafana-style) [/telemetry <port>|off]",
        ...
        "  /prune          Trim idle lead context without a model call: [dupes|tools|stale]; bare = all",
        "                  Keeps recent context and a recovery snapshot; delegated sessions are unchanged; prompt-cache reuse may decrease",
        ...
        "  /context <size> Set the session context window (auto-compact threshold): tokens, 64k/1m, or 'max' (queries the provider; no-op when not exposed)",
        "  /tokens all     Per-model matrixed cost/token breakdown (alias of /cost all)",
        "  /effort         Cycle live effort: low/med/high/xhigh/max (also Shift+Tab)",
        "  /effort xhigh   Select extra-high reasoning (wire xhigh, distinct from max)",
        "  /effort max     Select real provider max; /max is a shortcut (no silent fallback)",
        "  /effort custom <raw-value>  Send a provider effort string unchanged (no silent fallback)",
        ...
        "  /model          View selected lead and configured agent models",
        "  /setmodel       Change the model for any agent, orchestrator, or compaction agent in your swarm config",
        "  /swap           Fuzzy-search the lead for the next /agent session; Enter choose, Esc cancel (TUI)",
        ...
        "  /set            Search/edit settings in a native view: Enter edit, F4 Apply, Esc cancel. /set <key> opens its editor",
        "                  /set <key> <value> keeps direct validation/save; one setting per Apply, no automatic /refresh",
        ...
        "  /proxy          Manage the CLIProxyAPI sidecar: /proxy status | update [pinned] | restart",
        ...
        "  Ctrl+V / Alt+V  Paste text or screenshot; /paste is the terminal-safe fallback",
        "  Ctrl+Z          Undo the last attachment insertion/removal (before further text edits)",
        "  F2              Paste cards: arrows select, Enter preview, Delete detach, Esc return",
        "  PgUp / PgDn     Scroll an expanded paste preview (also arrows / mouse wheel)",
        ...
        "  Shift+Tab       Cycle reasoning effort (low/med/high/xhigh/max); custom returns to low",
        "",
        "Mouse (frame engine, default; /mouse off|wheel|buttons adjusts)",
        "  Wheel           Scroll the transcript viewport",
        "  Click           Select items/lanes/jobs/settings; position the compose caret; expand/collapse cards",
        "  Double-click    Activate the clicked item (the Enter alias)",
        "  Triple-click    Apply staged changes in /setmodel and /set (the F4 alias)",
        "  Drag            Select transcript lines; release copies clean text (works mid-turn)",
        "  Scrollbar       Click the rail to page; drag the thumb to scroll",
        "  Footer chips    Click the model chip for /setmodel; the effort chip cycles effort",
        "  Inline engine   Mouse stays native (terminal selection/scrollback); modals still clickable",
        "",
        "CLI Usage",
        "  mux-swarm \"<goal>\"",
        "  mux-swarm <goal.txt>",
        ...
        "  --agent <name>             Use the specified lead in the agentic interface instead of the swarm coordinator",
        "  --plan                     Enable plan mode, agents confirm before executing (coordinators and agentic lead sessions)",
        ...
        "  --sub / --subagents        Enable specialist delegation from the lead agent",
        ...
        "  --stateless                Boot straight into a stateless agentic session",
        "  --teams                    Boot straight into teams mode",
        "  --agent-mode               Boot straight into the main agentic interface (pair with --agent <name>)",
        "  --workflow <file>          Run deterministic and replayable workflows with control of the entire mux-swarm runtime",
