...
public static class CliCmdUtils
{
    ...
    public static void HandleSandbox(string userInput, string cfgPath)
    {
        ...
        if (parts.Length < 2)
        {
            ...
            if (!string.Equals(s.Backend, "host", StringComparison.OrdinalIgnoreCase))
            {
                ...
                var mounts = SandboxBackend.ResolveMounts(App.Config.Filesystem);
                if (mounts.Count > 0)
                {
    ...
    public static async Task HandleLoginAsync(string userInput, string cfgPath)
    {
        ...
        var supported = CliProxyManager.LoginProviders.Keys.ToList();
        ...
        if (!CliProxyManager.LoginProviders.ContainsKey(providerId))
        {
        ...
        try
        {
            ...
            bool ok = await CliProxyManager.LoginAsync(providerId, cts.Token, headless);
            if (!ok)
            {
            ...
            bool live = await CliProxyManager
            ...
            if (added)
                MuxConsole.WriteSuccess($"Logged in. Registered local provider 'cliproxy' -> {CliProxyManager.OpenAiEndpoint}.");
            else
    ...
    private static bool RegisterCliProxyProvider(string cfgPath)
    {
        ...
        if (existing is not null)
        {
            ...
            existing.ApiKeyEnvVar = CliProxyManager.ClientKeyEnvVar;
            File.WriteAllText(PlatformContext.ConfigPath, JsonSerializer.Serialize(config, CfgSerialOpts));
            App.Config = LoadConfig(cfgPath);
            return false;
        }
    ...
    public static async Task HandlePingAsync(string userInput)
    {
        ...
        try
        {
            ...
            await CliProxyManager.EnsureRunningAsync(cts.Token);
            sw.Stop();
            MuxConsole.WriteInfo($"  cliproxy sidecar: OK  {sw.ElapsedMilliseconds}ms  ({CliProxyManager.OpenAiEndpoint})");

            var files = await CliProxyManager.GetAuthFilesAsync(cts.Token);
            if (providerId is not null)
            {
                bool ready = await CliProxyManager.IsProviderReadyAsync(providerId, cts.Token);
    ...
    public static async Task HandleProxyAsync(string userInput)
    {
        ...
        try
        {
            ...
            if (sub is "restart" or "recycle")
            {
                ...
                using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(30));
                bool ok = await CliProxyManager.RecycleAsync(cts.Token);
                if (ok) MuxConsole.WriteSuccess($"Sidecar restarted -> {CliProxyManager.OpenAiEndpoint}.");
            ...
            {
                MuxConsole.WriteMuted($"  running: yes -> {CliProxyManager.OpenAiEndpoint}");
    ...
    {
        var s = ReflectionGatherer.Status();
        var sb = new System.Text.StringBuilder();
        ...
        sb.AppendLine($"reflections     : {s.count} / {s.max} max  in {ReflectionStore.Directory}");
    ...
    private static void SetMemoryOption(string? key, string? rawValue)
    {
        ...
        try
        {
            ...
            swarm.ReflectionAgent ??= new ReflectionConfig();
    ...
    private static void SetMemoryMode(string mode)
    {
        try
        {
            ...
            if (mode == "deep")
            {
                bool chroma = ReflectionStore.ChromaAvailable();
                bool kg = ReflectionStore.KgAvailable();
    ...
    public static void HandleTheme(string userInput)
    {
        ...
        var theme = Theme.Find(arg);
        if (theme is null)
        {
            MuxConsole.WriteWarning($"Unknown theme '{arg}'. Available: {string.Join(", ", Theme.Names)}");
            return;
        }
    ...
    {
        MuxConsole.WriteInfo($"Themes (active: {Theme.Active.Name}). Apply with /theme <name>:");
        foreach (var t in Theme.Presets)
        {
            string mark = t.Name == Theme.Active.Name ? "*" : " ";
            string swatch =
                $"[{t.Banner}]\u2588\u2588[/]" +
                $"[{t.Accent}]\u2588\u2588[/]" +
                $"[{t.Success}]\u2588\u2588[/]" +
    ...
    private static void RenderThemePreview(Theme t)
    {
