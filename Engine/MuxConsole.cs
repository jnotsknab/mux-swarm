...
public static partial class MuxConsole
{
    ...
    /// <list type="bullet">
    /// <item><see cref="Engine.RenderMode.Stdio"/> - reported when <see cref="StdioMode"/> is on.</item>
    /// <item><see cref="Engine.RenderMode.Tui"/> - full-screen live renderer (interactive default on a capable TTY).</item>
    /// <item><see cref="Engine.RenderMode.Classic"/> - pre-v0.11.0 line-by-line renderer (opt-out / non-capable fallback).</item>
    /// </list>
    /// </summary>
    ...
    /// <see cref="Engine.RenderMode.Stdio"/> whenever <see cref="StdioMode"/> is set.
    /// </summary>
    ...
    public static PromptChoice SelectChoice(string title, IEnumerable<string> choices)
    {
        ...
        // which is EXACTLY the bailout the affordance row emulated on the legacy path.
        {
            ...
            if (TuiPromptModal(PromptModalView.Kind.Select, title, modalRows) is { } pm)
            {
    ...
    public static PromptChoice ConfirmChoice(string message, bool defaultValue = true)
    {
        ...
        if (TuiPromptModal(PromptModalView.Kind.Select, message,
    ...
    public static PromptChoice MultiSelectChoice(string title, IEnumerable<string> choices)
    {
        ...
        {
            var modalRows = new List<string>(list) { CustomAffordanceLabel };
            if (TuiPromptModal(PromptModalView.Kind.MultiSelect, title, modalRows) is { } pm)
            {
    ...
    public static string Prompt(string message, string? defaultValue = null, bool secret = false)
    {
        ...
        if (!UsingDelimiter &&
            TuiPromptModal(PromptModalView.Kind.Text, message, null, defaultValue, secret) is { } pmT)
    ...
    public static string PromptSecret(string message)
    {
        ...
        if (TuiPromptModal(PromptModalView.Kind.Text, message, null, null, secret: true) is { } pmS)
    ...
    public static string Select(string title, IEnumerable<string> choices)
    {
        ...
        if (selList.Count > 0 &&
            TuiPromptModal(PromptModalView.Kind.Select, title, selList) is { } pmSel)
    ...
    public static List<string> MultiSelect(string title, IEnumerable<string> choices)
    {
        ...
        if (msList.Count > 0 &&
            TuiPromptModal(PromptModalView.Kind.MultiSelect, title, msList) is { } pmMs)
        {
